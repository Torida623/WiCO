...
// アレルギー・トーン・自然な組み合わせは安全/品質に関わる指示なので、直すときは1箇所で直せば
// 両方のプロンプトに反映される。確定用（材料・作り方の生成）は対象が「献立の品目選び」ではなく
// 「決まった品目の材料」なので、アレルギー指示の内容自体が異なり共通化していない。
const PEROKOKO_PERSONA_INTRO = 'あなたは家庭料理の献立を提案するアプリ「WiCO」のマスコット「ペロココ」です。';
...
const PROPOSAL_SYSTEM_PROMPT = `${PEROKOKO_PERSONA_INTRO}
ユーザーは日々の食事作りに追われ、時間や心に余裕がない人です。${TONE_INSTRUCTION}
...
const REVISION_SYSTEM_PROMPT = `${PEROKOKO_PERSONA_INTRO}
...
const FINAL_SYSTEM_PROMPT = `${PEROKOKO_PERSONA_INTRO}
...
ご飯の量は、単品の主食としてでも、丼・チャーハン・カレーのように他の品目の一部としてでも、「適量」としてください（「150g」「茶碗1杯分」のような厳密な分量にしないでください）。ご飯に限らず、厳密でなくてよい分量は「適量」としてください。このアプリはカロリー管理アプリではないため、すべての分量を厳密なグラム数で示す必要はありません。ただし丼・チャーハンのようにご飯に直接味付けする料理では、調味料の分量はご飯1人前200〜250g程度を想定して計算し、薄すぎたり濃すぎたりしないようにしてください（表示するご飯の分量自体は「適量」のままにしてください）。
